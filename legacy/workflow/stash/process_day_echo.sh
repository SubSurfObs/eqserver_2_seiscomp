...
# --- Ensure cleanup always runs on exit ---
cleanup() {
  if [ $KEEP_TEMP -eq 0 ] && [ -n "$temp_day_dir" ] && [ -d "$temp_day_dir" ]; then
    rm -rf "$temp_day_dir"
    [ $VERBOSE -eq 1 ] && echo "Temporary directory cleaned automatically: $temp_day_dir"
  fi
}
trap cleanup EXIT
...
log "Finished processing day: $(basename "$day_dir")"
