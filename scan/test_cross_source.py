...
# ---------- Tier 0: dispatcher-glitch detection (multi-variant telemetry) ----------

def test_disk_only_mode_activates_when_multi_tele_AND_disk_complete():
    """The STBK 2022-10-23 dispatcher-glitch case: per slot there is one
    disk + TWO overlapping telemetry variants (tele_ss and tele_noss).
    Disk-only mode activates because (a) every slot has a disk complete
    candidate AND (b) at least one slot has multi-variant telemetry.
    Both telemetry variants are dropped — disk wins."""
    rows = []
    for hhmm in ("0001", "0002", "0003"):
        rows.append(row(f"disk/{hhmm}.dmx", "disk", hhmm, size=24000))
        rows.append(row(f"tele_ss/{hhmm}.dmx", "telemetry", hhmm, size=54000))
        rows.append(row(f"tele_noss/{hhmm}.dmx", "telemetry", hhmm, size=54000))
    out = select_files_for_day(rows)
    assert out == ["disk/0001.dmx", "disk/0002.dmx", "disk/0003.dmx"], out


def test_disk_only_mode_does_NOT_activate_with_single_tele_per_slot():
    """The documented recorder-cut-out scenario: disk + ONE tele per slot.
    Tier 0 does NOT activate (no multi-variant signal). Tier 2 runs and
    applies the size-ratio rule as before — preserves the legitimate
    'tele has materially more data than disk' decision."""
    rows = [
        row("disk/0001.dmx", "disk", "0001", size=25000),
        row("tele/0001.dmx", "telemetry", "0001", size=50000),  # tele 2x larger
    ]
    out = select_files_for_day(rows)
    # Without tier 0 firing, tier 2 sees disk_size (25k) < 0.8 * tele_size (50k)
    # → tele wins (the recorder-cut-out case).
    assert out == ["tele/0001.dmx"], out


def test_disk_only_mode_with_multi_tele_picks_largest_disk():
    """When tier 0 fires, the per-slot disk filter still applies the
    existing _pick_complete logic to the disk-only candidates."""
    rows = [
        row("disk_a/0001.dmx", "disk", "0001", size=10000),
        row("disk_b/0001.dmx", "disk", "0001", size=20000),  # largest
        row("tele_ss/0001.dmx", "telemetry", "0001", size=54000),
        row("tele_noss/0001.dmx", "telemetry", "0001", size=54000),
    ]
    out = select_files_for_day(rows)
    assert out == ["disk_b/0001.dmx"], out


def test_disk_only_mode_does_NOT_activate_if_disk_missing_a_slot():
    """If disk is missing for any slot, disk-only mode cannot be safely
    used (we'd drop telemetry from a slot with no disk fallback). Tier 0
    refuses to activate; tier 2 runs per-slot."""
    rows = [
        # slot 0001: disk + 2 tele variants
        row("disk/0001.dmx", "disk", "0001", size=24000),
        row("tele_ss/0001.dmx", "telemetry", "0001", size=54000),
        row("tele_noss/0001.dmx", "telemetry", "0001", size=54000),
        # slot 0002: ONLY tele — disk gap, blocks disk-only mode
        row("tele/0002.dmx", "telemetry", "0002", size=54000),
    ]
    out = select_files_for_day(rows)
    # disk-only NOT activated. Slot 0001: tier 2, disk_size (24k) < 0.8 * tele (54k)
    # → tele wins (one of them; whichever is largest — both equal so the first).
    # Slot 0002: only one telemetry candidate → use it.
    # We don't assert which tele variant wins in slot 0001 (depends on dict
    # ordering); just that the output has 2 picks neither of which is disk.
    assert len(out) == 2
    assert "disk/0001.dmx" not in out  # disk-only mode did not fire


def test_disk_only_mode_does_NOT_activate_if_disk_is_only_stubs():
    """A slot where disk is present only as single-channel stubs (no
    complete file) does NOT enable disk-only mode — 'complete' coverage
    requires channel_suffix=None disk files at every slot."""
    rows = [
        # slot 0001: disk has only a stub, multi-variant tele complete present
        row("disk_stub/0001_Z.mseed.zip", "disk", "0001", suffix="Z", size=15000),
        row("tele_ss/0001.dmx", "telemetry", "0001", size=54000),
        row("tele_noss/0001.dmx", "telemetry", "0001", size=54000),
    ]
    out = select_files_for_day(rows)
    # disk-only NOT enabled (no disk complete at slot 0001).
    # Tier 2 picks tele (largest of the two complete tele candidates;
    # ties broken by path — both equal so 'tele_noss' wins lexicographically).
    assert len(out) == 1
    assert out[0].startswith("tele_")
