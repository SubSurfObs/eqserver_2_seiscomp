...
## Per-network sweep completion (provenance snapshot)

The **final step** for each per-network sweep is a provenance snapshot
on the shared staging mediaflux share. One snapshot per network per
sweep, dated; lets us re-derive any unit-year's history later or audit
"what was the engine state when X was converted?" without spelunking
through scattered logs and orphaned scratch directories.

**Path convention** (network + date):

```
/mnt/seiscomp_staging/eqserver_provenance/<NETWORK>/<YYYY-MM-DD>/
```

**Generated by** `scan/build_provenance_snapshot.sh <NETWORK>`. The
script captures:

| Subdir / file | What |
|---|---|
| `jsonl/` | All orchestrator state files (convert_done, promote_done, held, convert_failed, cleanup_done) |
| `plans/` | Per-station plan YAMLs (the conversion gate) |
| `run_manifests/` | One JSON per (sta, year) with `policy_sha` + `project_git` + per-day status |
| `convert_logs/` | Per-station-year phase3 stdout (heartbeats, timeouts, day-by-day) |
| `recovery_logs/` | `run_recovery_register.py` per-entry logs |
| `recovery_register.yaml` | The recovery doc itself |
| `handoffs/` | Cross-project handoff threads (disk_to_sds, uom_seismic_metadata) |
| `metadata/` | `level2_metadata_scan.py` YAML (the empirical waveform-db source for `uom_seismic_metadata`) |
| `station_db_manifest.csv` | Audit listing of Level-1 station DBs (names + sizes + mtimes) |
| `station_dbs/` | The actual Level-1 SQLite DBs (rsync'd in background — ~60 GB for VW) |
| `code_sha.txt` | Git SHAs at snapshot time + recovery + held summary |
| `README.md` | What's in here, how to use, conventions reference |

**Why this works:**
- The mediaflux backend is soft-delete-recoverable for 1 year, versioned
  overwrites, and offsite-replicated within seconds, per the
  "Mediaflux service properties" section. So a snapshot survives
  accidents.
- The 30 TB auto-grant means we don't need to worry about storage cost
  per snapshot. VW's snapshot is ~60 GB; VX and DU will be smaller.
- The provenance dir lives alongside (not inside) the `seiscomp_archive/`
  and `eqserver_sweep/` subdirs on the staging share, keeping the
  active-sweep state separate from the durable archive of past sweeps.

**When to run:** after the convert sweep is complete, promote has caught
up, the recovery register is cleared, and the metadata YAML
(`level2_metadata_scan.py`) has been generated for the network. The
snapshot is the marker that says "this sweep is finished; future work
on this network starts a new dated snapshot."

**Per network so far:**
- `VW/2026-06-10/` — current sweep (VW completion). First snapshot.
- `VX/` — pending VX sweep launch.
- `DU/` — pending DU sweep launch.
